...
object DynVerPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val dynverGitDescribeOutput =
      settingKey[Option[GitDescribeOutput]]("The output from git describe")
    val dynverSonatypeSnapshots =
      settingKey[Boolean]("Whether to append -SNAPSHOT to snapshot versions")
...
final case class GitTagPrefix(value: String)
...
object GitCommitSuffix extends ((Int, String) => GitCommitSuffix) {
  final implicit class GitCommitSuffixOps(val x: GitCommitSuffix) extends AnyVal {
    import x._
...
final case class GitDescribeOutput(
  ...
  def version(sep: String): String = {
    ...
    else if (commitSuffix.sha.nonEmpty)
      ref.dropV.value + sep + commitSuffix.distance + "-" + commitSuffix.sha + dirtySuffix
    else "0.0.0" + sep + commitSuffix.distance + "-" + ref.value + dirtySuffix
  }

  def sonatypeVersion(sep: String): String =
    if (isSnapshot) version(sep) + "-SNAPSHOT" else version(sep)
...
object GitDescribeOutput
    extends ((GitTagPrefix, GitRef, GitCommitSuffix, GitDirtySuffix) => GitDescribeOutput) {
  ...
  private val FromPrefixedTag = s"""^$Prefix$OptWs$Tag$CommitSuffix?$TstampSuffix?$OptWs$$""".r
  ...
  private[sbtdynver] def parse: String ?=> GitDescribeOutput = {
    ...
    case FromPrefixedTag(prefix, tag, _, dist, sha, dirty) => parse0(prefix, tag, dist, sha, dirty)
    case FromSha(sha, dirty)                               => parse0("", sha, "0", "", dirty)
    case FromHead(dirty)                                   => parse0("", "HEAD", "0", "", dirty)
  }

  private def parse0(prefix: String, ref: String, dist: String, sha: String, dirty: String) = {
    val commit =
      if (dist == null || sha == null) GitCommitSuffix(0, "") else GitCommitSuffix(dist.toInt, sha)
    GitDescribeOutput(
      GitTagPrefix(prefix),
      GitRef(ref),
      commit,
...
sealed case class DynVer(prefix: String, wd: Option[File], separator: String) {
  private def this(prefix: String, wd: Option[File]) = this(prefix, wd, "+")
  ...
  def copy(prefix: String, wd: Option[File] = wd): DynVer = new DynVer(prefix, wd, separator)
}

object DynVer extends DynVer("", None) with ((String, Option[File]) => DynVer) {
  override def apply(prefix: String, wd: Option[File]) = apply(prefix, wd, separator)
}

...
class LdapLoginAuthenticationDomain(ldapConfig: LdapConfig) extends LoginDomain {
  ...
  private[this] def fromResultCodeToLDAPError(result: ResultCode): AuthenticationError =
    result match {
      ...
      case ResultCode.UNAVAILABLE_CRITICAL_EXTENSION =>
        AuthenticationError.LDAPUnavailableCriticalExtension
      ...
      case ResultCode.ALIAS_DEREFERENCING_PROBLEM =>
        AuthenticationError.LDAPAliasDereferencingProblem
      case ResultCode.INAPPROPRIATE_AUTHENTICATION =>
        AuthenticationError.LDAPInappropriateAuthentication
      ...
      case ResultCode.OBJECT_CLASS_MODS_PROHIBITED =>
        AuthenticationError.LDAPObjectClassModsProhibited
      ...
      case ResultCode.INTERACTIVE_TRANSACTION_ABORTED =>
        AuthenticationError.LDAPInteractiveTransactionAborted
      case ResultCode.DATABASE_LOCK_CONFLICT => AuthenticationError.LDAPDatabaseLockConflict
      case ResultCode.MIRRORED_SUBTREE_DIGEST_MISMATCH =>
        AuthenticationError.LDAPMirroredSubtreeDigestMismatch
      case ResultCode.TOKEN_DELIVERY_MECHANISM_UNAVAILABLE =>
        AuthenticationError.LDAPTokenDeliveryMechanismUnavailable
      case ResultCode.TOKEN_DELIVERY_ATTEMPT_FAILED =>
        AuthenticationError.LDAPTokenDeliveryAttemptFailed
      case ResultCode.TOKEN_DELIVERY_INVALID_RECIPIENT_ID =>
        AuthenticationError.LDAPTokenDeliveryInvalidRecipientId
      case ResultCode.TOKEN_DELIVERY_INVALID_ACCOUNT_STATE =>
        AuthenticationError.LDAPTokenDeliveryInvalidAccountState
      case _ => AuthenticationError.LDAPGenericError
    }
}

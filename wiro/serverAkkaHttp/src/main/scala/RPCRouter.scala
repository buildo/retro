...
import io.circe.{Json, JsonObject, ParsingFailure, Printer}
...
trait Router extends RPCServer with PathMacro with MetaDataMacro with LazyLogging {
  ...
  private[this] def operationName(
    operationFullName: String,
    methodMetaData: MethodMetaData,
  ): String =
    methodMetaData.operationType.name.getOrElse(operationPath(operationFullName).last)

  private[this] def autowireRequest(
    operationFullName: String,
    args: Map[String, Json],
  ): autowire.Core.Request[Json] =
    autowire.Core.Request(path = operationPath(operationFullName), args = args)

  private[this] def autowireRequestRoute(
    operationFullName: String,
    args: Map[String, Json],
  ): Route =
    Either
      .catchNonFatal(routes(autowireRequest(operationFullName, args)))
      .fold(handleUnwrapErrors, result => complete(result))

  private[this] def autowireRequestRouteWithToken(
    operationFullName: String,
    args: Map[String, Json],
  ...
  private[this] def loggingClientIP: Directive0 = extractClientIP.map { ip =>
      ...
      (routePathPrefix(operationFullName, methodMetaData) & pathEnd & get & parameterMap) {
        params =>
          headersDirective { headers =>
            val args = params.mapValues(parseJsonObjectOrString).toMap

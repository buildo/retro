...
trait CirceSupport {
  ...
  private[this] def deserializationError(actual: String,
                                         expected: String): String =
    s"Expected a value of type ${expected}, but got ${actual}"

  private[this] def deserializationErrorForIndex(actual: String,
                                                 expected: String): String =
    s"Expected an indexed value for type ${expected}, but got ${actual}"

  implicit def encodeCaseEnum[E <: CaseEnum](
      implicit s: CaseEnumSerialization[E]): Encoder[E] =
    Encoder.encodeString.contramap[E](s.caseToString)

  implicit def decodeCaseEnum[E <: CaseEnum](
      implicit s: CaseEnumSerialization[E]): Decoder[E] =

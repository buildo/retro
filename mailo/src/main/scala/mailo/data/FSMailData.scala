...
import FSMailDataError._
  ...
  private[this] def internalLoadPartials(): Map[String, String] = {
    Try(directory.listFiles(f => f.isDirectory && f.getName.equalsIgnoreCase("partials")).headOption)
      .map(_.map(
        partials =>
          partials.listFiles(f => f.isFile).map(file => {
            val buffer = scala.io.Source.fromFile(file)
            val template = Try(buffer.mkString)
            buffer.close()
            template.map(content => (file.getName, content))
          }).flatMap(_.toOption).toMap //convert all tries to an option, remove none and create a map with the remainings
      )).toOption.flatten.getOrElse(Map.empty[String, String])
  }
  ...
  private[this] def internalGet(name: String) = {
    ...
    else directory.listFiles().find(f => f.getName == name).map(
      templateFile => parseTemplateFile(templateFile)
    ).getOrElse(Left(TemplateNotFound(directory.getAbsolutePath.concat(s"/$name"))))
  }

  override def get(name: String): Future[Either[MailError, MailRawContent]] = Future.successful(internalGet(name))
}
